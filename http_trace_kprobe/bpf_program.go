...
int syscall__probe_ret_read(struct pt_regs* ctx) {
  ...
  if (read_args != NULL) {
    ...
    if (active_fds.lookup(&read_args->fd) == NULL) {
      // Bail early if we aren't tracking fd.
      active_read_args_map.delete(&id);
      return 0;
    }
  
    event->attr.fd = read_args->fd;
    event->attr.bytes = bytes_count;
    size_t buf_size = bytes_count < sizeof(event->msg) ? bytes_count : sizeof(event->msg);
    bpf_probe_read(&event->msg, buf_size, (void*) read_args->buf);
    event->attr.msg_size = buf_size;
