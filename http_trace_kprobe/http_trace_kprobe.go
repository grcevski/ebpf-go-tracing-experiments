...
func parseAndPrintMessage(msgInfo *MessageInfo) {
	...
	req, err := http.ReadRequest(bufio.NewReader(&msgInfo.ReadBuf))
	if err != nil {
		if errPrintEnabled {
	...
	if printEnabled {
		...
		body.Close()
		fmt.Printf("URI: %s, StatusCode: %s, Len: %s, ContentType: %s, Body: %s\n",
			color.GreenString("%q", req.URL.String()),
...
func main() {
	...
	mustAttachKprobeToSyscall(bccMod, bcc.BPF_PROBE_ENTRY, "read", "syscall__probe_entry_read")
	mustAttachKprobeToSyscall(bccMod, bcc.BPF_PROBE_RETURN, "read", "syscall__probe_ret_read")
